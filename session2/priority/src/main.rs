...
fn low_prio() {
    ...
    loop {
        ...
        std::thread::yield_now(); // done with current iteration and may schedule another thread
    }
}
...
// thread priority is less controversial than affinity but one issue with priority is it's never enterily clear exactly what it'll do
